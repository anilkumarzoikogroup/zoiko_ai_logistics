...
Agent Runtime flow:
  ...
  4..N. RULE_ENGINE:<rule_name> — one step per SC-002 rule
...
SC-002's rule formula is deterministic and must never change once shipped:
  liability_acknowledged (0.95 × 0.55) + amount_within_policy_cap (0.90 × 0.45) = 0.9275
...
class AgentRuntime:
    """Orchestrates tool calls and produces a reasoning trace for SC-002."""
    ...
    def analyze(
        ...
        try:
            ...
            """, (
                ...
                f"SC002 weighted confidence {confidence}: recommending {proposed_action}",
            ))
