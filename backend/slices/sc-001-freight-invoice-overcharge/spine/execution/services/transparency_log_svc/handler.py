"""
Transparency Log Service — append-only Merkle log over issued ACRs.

Every ACR appended here gets batched (immediately, per-tenant) into a
Merkle tree whose root is signed and persisted as a "commit". Anyone
holding a leaf's inclusion proof + the published root can independently
verify the ACR was logged, without needing live database access — that's
the property the SC-001 acceptance criteria call "transparency log
inclusion proof and witness co-signature verify".

LIMITATION (flagged, not hidden): the witness signature is produced with
the same signing key/process used everywhere else in this codebase
(shared.signer.sign). A real witness co-signature requires an independent
key custodian — once that exists, swap the signer call in
_commit_pending() for the witness's signer. Calling this signature
"independent" today would be dishonest.
"""
...
class InclusionProof:
    ...
    root_hash:          str   # hex
    commit_id:          str
    proof:              dict  # MerkleProof.to_dict()
    witness_signature:  str   # hex
    witness_kid:        str
...
class TransparencyLogHandler:
    ...
    def append(self, tenant_id: str, acr_id: str, acr_merkle_root_hex: str) -> str:
        """Append one ACR's hash to the log, then commit it. Returns the
        transparency_log_entries.id. Best-effort callers should catch
        exceptions — this must never block ACR issuance."""
        ...
        try:
            ...
            # Serialize log_index assignment per tenant
    ...
    def _commit_pending(self, tenant_id: str) -> Optional[str]:
        """Batch every uncommitted entry for this tenant into one Merkle tree
        and sign the root. Safe to call repeatedly — no-ops if nothing pending."""
        tenant_id = str(tenant_id)
        conn = psycopg2.connect(self._db_url)
        try:
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            # Leaves are already domain-tagged hashes from append() — build the
            # tree directly over them rather than re-hashing through .append().
    ...
    def get_inclusion_proof(self, tenant_id: str, acr_id: str) -> InclusionProof:
        ...
        try:
            ...
            if not entry["commit_id"]:
                raise ValueError(f"Transparency log entry for ACR '{acr_id}' has not been committed yet")
    ...
    def verify_inclusion(root_hash_hex: str, leaf_hash_hex: str, proof_dict: dict) -> bool:
        """Independent check — needs only the root, leaf hash, and proof.
        No database access required, matching the SC-001 'independent
        verifier procedure passes without platform-runtime access' criterion."""
