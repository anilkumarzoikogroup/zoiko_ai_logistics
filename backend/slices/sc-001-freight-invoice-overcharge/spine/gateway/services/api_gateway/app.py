...
def _auth_cookie_response(data: dict, token: str, ttl: int, status_code: int = 200):
    ...
    resp.set_cookie(
        ...
        secure=True,          # required by SameSite=None; localhost is a secure-context exception
        samesite="none",      # frontend and backend are always on different domains (Vercel + Render)
        max_age=ttl,
        path="/",
    )
    return resp
...
from services.api_gateway import routes_logic as _sc001_routes
...
def ingest_invoice(
    ...
    return InvoiceResponse(**_sc001_routes.ingest_invoice(_ingestion, str(claims.tenant_id), body, idempotency_key))
...
def validate_invoice(
    ...
    return ValidateResponse(**_sc001_routes.validate_invoice(_validation, str(claims.tenant_id), source_record_id, body))
...
def canonicalize_invoice(
    ...
    return CanonicalizeResponse(**_sc001_routes.canonicalize_invoice(_canonical, str(claims.tenant_id), source_record_id, body))
...
def open_case(
    ...
    return OpenCaseResponse(**_sc001_routes.open_case(_cases, str(claims.tenant_id), body.canonical_invoice_id, claims.sub))
...
def ui_list_cases(
    ...
    return _sc001_routes.ui_list_cases(_r, claims.tenant_id, state, page, page_size)


@v1_router.get("/cases/{case_id}", tags=["ui"])
def ui_get_case(case_id: str, claims: ZoikoClaims = Depends(get_claims)):
    return _sc001_routes.ui_get_case(_r, claims.tenant_id, case_id)
...
def ui_validation(case_id: str, claims: ZoikoClaims = Depends(get_claims)):
    return _sc001_routes.ui_validation(_r, claims.tenant_id, case_id)
...
def ui_canonical_invoice(case_id: str, claims: ZoikoClaims = Depends(get_claims)):
    return _sc001_routes.ui_canonical_invoice(_r, claims.tenant_id, case_id)
...
from services.api_gateway import parse_invoice as _sc001_parse_invoice
...
async def parse_invoice_file(
    ...
    """SC-001 — see services/api_gateway/parse_invoice.py."""
    ...
    return await _sc001_parse_invoice.parse_invoice_file(content, file.filename, file.content_type)
...
def _run_full_pipeline(
    ...
    """SC-001 — see services/api_gateway/routes_logic.py:run_full_pipeline()."""
    return _sc001_routes.run_full_pipeline(
        DB_URL, IngestionHandler, ValidationHandler, CanonicalHandler, CaseHandler,
        _run_evidence_and_reasoning,
        tenant_id, actor_sub, carrier, origin, dest, amount, currency,
        invoice_number=invoice_number, channel=channel, channel_metadata=channel_metadata,
    )
...
async def batch_submit_invoices(
    ...
    for f in files:
        ...
        # No cross-file rollback needed; each run_full_pipeline is its own transaction.
        item = _sc001_routes.process_one_batch_file(
            _run_full_pipeline, content, f.filename, f.content_type,
...
def generate_dispute_letter(case_id: str, claims: ZoikoClaims = Depends(get_claims)):
    """SC-001 — see services/api_gateway/routes_logic.py."""
    return _sc001_routes.generate_dispute_letter(claims.tenant_id, claims.sub, case_id)
...
def _run_evidence_and_reasoning(
    ...
    """SC-001 — see services/api_gateway/routes_logic.py:run_evidence_and_reasoning()."""
    return _sc001_routes.run_evidence_and_reasoning(
        DB_URL, tenant_id, case_id, slug, carrier, amount, currency, route, actor_sub, broker,
    )
...
def ui_submit_case_async(
    ...
    SC-001 — worker body in services/api_gateway/routes_logic.py.
    """
    ...
    _th.Thread(
        target=_sc001_routes.submit_case_async_worker,
        args=(
            DB_URL, _BROKER, IngestionHandler, ValidationHandler, CanonicalHandler, CaseHandler,
            _run_evidence_and_reasoning, _SUBMIT_JOBS, _persist_job,
            job_id, str(claims.tenant_id), claims.sub, idempotency_key, body,
        ),
        daemon=True, name=f"submit-{job_id[:8]}",
    ).start()
...
def ui_submit_case(
    ...
    SC-001 — see services/api_gateway/routes_logic.py:submit_case().
    """
    from services.api_gateway.routers.ingestion import _capture_rest_push_metadata
    return _sc001_routes.submit_case(
        DB_URL, _BROKER, IngestionHandler, ValidationHandler, CanonicalHandler, CaseHandler,
        _run_evidence_and_reasoning, _capture_rest_push_metadata,
