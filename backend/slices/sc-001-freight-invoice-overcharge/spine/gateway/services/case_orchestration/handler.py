...
class CaseHandler:
    ...
    def open_case(
        ...
        """Open a case against an invoice."""
        tenant_id = str(tenant_id)
        case_type = "INVOICE_OVERCHARGE"
        ...
        try:
            ...
            # UNIQUE per subject (tenant_id, invoice_id) — idempotent
            case_id = uuid.uuid4()
            cur.execute("""
                INSERT INTO cases (id, tenant_id, invoice_id, case_type, state, opened_at)
                VALUES (%s, %s, %s, %s, 'NEW', %s)
                ON CONFLICT (tenant_id, invoice_id) WHERE invoice_id IS NOT NULL DO NOTHING
                RETURNING id, state, opened_at
            """, (case_id, tenant_id, canonical_invoice_id, case_type, now))
            row = cur.fetchone()
            ...
            if is_new:
                ...
                event_payload = {"invoice_id": str(canonical_invoice_id)}
                cur.execute("""
