"""
Transparency Log Service — append-only Merkle log over issued ACRs (SC-003).
...
verify the ACR was logged, without needing live database access.

Note: the witness signature uses the same signing infrastructure as the
rest of the platform (shared.signer.sign). A real independent witness
requires a separate key custodian — swap _commit_pending()'s signer call
when that exists.
"""
...
class TransparencyLogHandler:
    ...
    def verify_inclusion(root_hash_hex: str, leaf_hash_hex: str, proof_dict: dict) -> bool:
        """Independent check — no database access required."""
