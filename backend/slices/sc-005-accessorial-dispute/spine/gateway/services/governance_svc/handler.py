"""
SC-005 Governance Service — SoD-enforced proposal and approval for accessorial charge disputes.

Policy: accessorial-dispute-policy@2026.05.01
Proposed action: ISSUE_PARTIAL_CREDIT
Task type: APPROVE_ACCESSORIAL_DISPUTE
...
_POLICY_NAME    = "accessorial-dispute-policy"
_POLICY_VERSION = "2026.05.01"
_POLICY_ID      = f"{_POLICY_NAME}@{_POLICY_VERSION}"


class GovernanceHandler:
    def __init__(self, db_url: str, broker, tenant_slug: str = "default"):
        self.db_url      = db_url
        self.broker      = broker
    ...
    def propose(
        ...
        """Analyst proposes a partial credit for the accessorial dispute. Returns the task record."""
        ...
        proposal_dict = {
            ...
            "proposed_action": "ISSUE_PARTIAL_CREDIT",
        ...
        try:
            ...
            """, (
                ...
                "ISSUE_PARTIAL_CREDIT", amount, currency,
            ...
            """, (
                ...
                "APPROVE_ACCESSORIAL_DISPUTE", "PENDING_APPROVAL", actor_sub,
                json.dumps({
                    "amount": amount, "currency": currency,
                    "policy_id": _POLICY_ID, "finding_id": finding_id,
                }),
                _POLICY_ID, now,
            ))
            row = cur.fetchone()
            ...
            """, (
                ...
                json.dumps({"amount": amount, "currency": currency, "policy_id": _POLICY_ID}),
                now,
            ))
    ...
    def decide(
        ...
        """Manager approves or rejects the accessorial dispute credit. SoD enforced."""
        case_uuid = uuid.UUID(case_id)
        ...
        try:
            ...
            """, (
                ...
                decision_hash, d_sig, d_kid, now, _POLICY_ID,
            ))
            ...
            """, (
                ...
                json.dumps({"decision": decision, "note": note, "policy_id": _POLICY_ID}),
                now,
            ))
