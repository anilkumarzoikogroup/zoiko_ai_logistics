"""
SC-005 Execution Gateway — 8-gate check before ISSUE_PARTIAL_CREDIT is actioned.

Gates (same shape as SC-002/003/004/Phase-4):
  ...
  G5  Scope check         — allowed_actions includes ISSUE_PARTIAL_CREDIT
  G6  Sanctions screening — carrier not on watch-list
  G7  FX/rate check       — currency supported
  G8  Connector readiness — skipped in DEV_MODE

Action: ISSUE_PARTIAL_CREDIT
"""
import os
import uuid
import json
...
class ExecutionGatewayHandler:
    ...
    def _run_gates(self, tenant_id: str, case_id: str, token_id: str, action: str) -> list[str]:
        ...
        # G1 — governance token exists for this tenant
        token_row = q1(
        ...
        # G3 — Redis consumed lock (SET NX) prevents duplicate execution
        ...
        # G5 — action must match token scope (ISSUE_PARTIAL_CREDIT)
        scope = token_row["scope"] or ""
        if action != scope:
            errors.append(f"G5: action {action!r} does not match token scope {scope!r}")

        # G6/G7 — carrier + currency from canonical_invoices JOIN cases
        inv_row = q1(
            """SELECT ci.carrier_id, ci.currency
        ...
        carrier_name = (inv_row or {}).get("carrier_id") or ""
        if carrier_name and carrier_name in _SANCTIONED_CARRIERS:
            errors.append(f"G6: carrier {carrier_name!r} is on the sanctions watch-list")

        currency = (inv_row or {}).get("currency", "INR") or "INR"
    ...
    def _write_envelope(
        ...
        digest      = hashlib.sha256(b"zoiko.execution.envelope.v1:" + payload.encode())

        from shared.signer import sign
        sig_bytes, kid = sign("default", digest.digest())
    ...
    def _advance_case(self, tenant_id: str, case_id: str, actor_sub: str) -> None:
        ...
        q("""
            ...
            VALUES (gen_random_uuid(), %s::uuid, %s::uuid, 'STATE_TRANSITION',
                    ...
                    '{"action":"ISSUE_PARTIAL_CREDIT"}'::jsonb, NOW())
