"""
SC-005 Transparency Log Service — append-only Merkle log over issued ACRs.

Every ACR appended is batched per-tenant into a Merkle tree whose root is
signed and persisted as a commit. The inclusion proof allows independent
verification of ACR logging without live DB access.
"""
...
class TransparencyLogHandler:
    ...
    def append(self, tenant_id: str, acr_id: str, acr_merkle_root_hex: str) -> str:
        """Append one ACR to the log and commit. Returns entry id."""
