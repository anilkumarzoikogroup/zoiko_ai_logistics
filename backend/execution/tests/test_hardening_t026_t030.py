...
Phase 4: Reconciliation, WORM lock, Gate 3 consumed, Audit WORM index, SC-001 confidence.
...
T-030  SC-001 confidence is exactly 0.96 (deterministic formula)
"""
import uuid
import pytest
...
# ── T-030: SC-001 confidence = 0.96 (deterministic) ──────────────────────────

class TestT030SC001Confidence:
    """T-030: SC-001 weighted confidence formula is exactly 0.96 — immutable invariant."""
    ...
    def test_t030_confidence_formula_exact(self):
        ...
        assert computed == 0.96, f"SC-001 confidence must be 0.96, got {computed}"

    def test_t030_sc001_constant_from_module(self):
        from services.reasoning_svc.handler import SC001_CONFIDENCE
        assert SC001_CONFIDENCE == 0.96, (
            f"SC001_CONFIDENCE in reasoning_svc.handler must be 0.96, got {SC001_CONFIDENCE}"
        )
    ...
    def test_t030_changing_weights_changes_confidence(self):
        ...
        rules_wrong = {
            "fuel_charge": {"confidence": 1.00, "weight": 0.40},
            "accessorial": {"confidence": 0.92, "weight": 0.60},
        }
    ...
    def test_t030_confidence_is_not_rounded_up(self):
        """0.96 must not be a rounded result of a different formula."""
        rules = {
            "fuel_charge": {"confidence": 1.00, "weight": 0.50},
            "accessorial": {"confidence": 0.92, "weight": 0.50},
        }
        raw = sum(r["confidence"] * r["weight"] for r in rules.values())
        assert abs(raw - 0.96) < 1e-10   # exact floating-point match

    def test_t030_confidence_range_valid(self):
        from services.reasoning_svc.handler import SC001_CONFIDENCE
        assert 0.0 < SC001_CONFIDENCE <= 1.0
        assert SC001_CONFIDENCE == 0.96
