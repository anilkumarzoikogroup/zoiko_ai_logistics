...
from services.canonical_truth.models import CanonicalResult, CanonicalClaimResult
...
class CanonicalHandler:
    ...
    def canonicalize_claim(
        ...
        """SC-002 — mirrors canonicalize_invoice()'s hash/sign/lineage pattern
        but writes the existing `claims` table instead of canonical_invoices.
        Does not touch canonicalize_invoice()."""
        tenant_id        = str(tenant_id)
        source_record_id = uuid.UUID(str(source_record_id))
        canonical_dict = {
            "carrier_id":       carrier_id,
