    ...
    def ingest_invoice(
        ...
        *,
        ...
        try:
            ...
            ciphertext = canonical_bytes   # DEV_MODE — plaintext stand-in
        ...
        try:
            ...
            # Check dedup (SELECT only) before writing source_record so we know the
            # outcome to store on it.  The actual dedup_index INSERT must come AFTER
            # source_records is inserted because of the FK constraint.
            ...
            # Write dedup_index NOW — source_record row exists so FK is satisfied
            write_dedup_index(
                cur                 = cur,
                tenant_id           = tenant_id,
                dedup_key           = dedup_key,
            ...
            # Advance FSM: SIGNED → PENDING_VALIDATION
            ...
            # Route AMBIGUOUS records to the ambiguity queue
        ...
        # ── Step 7: Kafka publish AFTER commit (outbox relay recovers on crash) ──
        try:
